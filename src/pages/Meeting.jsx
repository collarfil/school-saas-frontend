...
import { Edit, Trash2, Plus, Eye, Calendar, Clock, Video, Users, Link, CheckCircle, XCircle, Play } from "lucide-react";
...
export default function Meeting() {
  ...
  const getTableData = () => {
    return meetings.map((meeting) => ({
      ...
      teacher: meeting.employee?.name || "N/A",
  ...
  const renderTableActions = (row) => (
    <div className="flex items-center justify-end gap-2">
      <button
        onClick={() => { setSelectedMeeting(row.original); setShowViewModal(true); }}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50 p-4">
          <div className="bg-slate-800 p-6 rounded-lg w-full max-w-2xl border border-slate-700 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <h4 className="text-2xl font-bold text-white">{selectedMeeting.title}</h4>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-slate-700/30 rounded-lg">
                <div>
                  ...
                  <p className="text-white">{selectedMeeting.employee?.name || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Meeting Provider</p>
                  <p className="text-white">{selectedMeeting.meeting_provider || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Start Time</p>
                  <p className="text-white">{formatDate(selectedMeeting.start_time)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">End Time</p>
                  <p className="text-white">{formatDate(selectedMeeting.end_time)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Max Participants</p>
                  <p className="text-white">{selectedMeeting.max_participants || "Unlimited"}</p>
                </div>
              </div>
