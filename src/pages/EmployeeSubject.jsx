...
export default function EmployeeSubject() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const mappingsRes = await api.get("/employee-subjects", {
        params: { school_id: schoolId }
      });
      
      console.log("✅ Employee-Subject mappings data:", mappingsRes.data);
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this mapping?")) {
      try {
        const schoolId = getSchoolId();
        await api.delete(`/employee-subjects/${id}`, {
          params: { school_id: schoolId }
        });
    ...
    <div className="p-6">
      ...
      <div className="mb-4 p-3 bg-slate-800 rounded text-sm">
        <div className="text-gray-300">
          ...
          <br />
          <strong>Subjects:</strong> {subjects.length} available
          <br />
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center">
              <p className="text-gray-400 mb-4">No employee-subject assignments found</p>
              <button
                onClick={openModal}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="py-4 px-6 text-left text-sm font-medium text-gray-300">Subject</th>
              ...
              <tbody>
                {mappings.map((mapping, index) => (
                  <tr key={mapping.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                    ...
                    <td className="py-4 px-6">
                      <div className="font-medium text-white">
                        {mapping.subject?.name || getSubjectName(mapping.subject_id)}
                      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 w-full max-w-md rounded-xl border border-slate-700">
            <div className="p-6 border-b border-slate-700">
              <h2 className="text-xl font-semibold text-white">Assign Subject to Employee</h2>
              <p className="text-gray-400 text-sm mt-1">Select an employee and subject to assign</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-700">
                ...
                <button
                  type="submit"
                  disabled={saveLoading || employees.length === 0 || subjects.length === 0}
