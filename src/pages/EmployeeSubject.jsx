...
export default function EmployeeSubject() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/employee-subjects", { ...form, school_id: schoolId });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <div><h1 className="text-2xl font-bold text-white">Employee-Subject Assignments</h1><p className="text-gray-400">Assign subjects to employees</p></div>
      ...
      <div className="mb-4 p-3 bg-slate-800 rounded text-sm">
        <div className="text-gray-300"><strong>School ID:</strong> {getSchoolId() || "Not found"}<br /><strong>Employees:</strong> {employees.length} available<br /><strong>Subjects:</strong> {subjects.length} available<br /><strong>Mappings:</strong> {mappings.length} assignments found</div>
      </div>
      ...
      {showModal && (<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div className="bg-slate-800 w-full max-w-md rounded-xl border border-slate-700">
          <div className="p-6 border-b border-slate-700"><h2 className="text-xl font-semibold text-white">Assign Subject to Employee</h2><p className="text-gray-400 text-sm mt-1">Select an employee and subject to assign</p></div>
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div><label className="block text-sm font-medium text-gray-300 mb-2">Select Subject <span className="text-red-400">*</span></label>
            ...
            {subjects.length === 0 && <p className="text-yellow-400 text-sm mt-2">No subjects found. Please add subjects first.</p>}</div>
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-slate-700">
              ...
              <button type="submit" disabled={saveLoading || employees.length === 0 || subjects.length === 0} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium disabled:bg-blue-400">{saveLoading ? "Saving..." : "Save Assignment"}</button>
            </div>
          </form>
        </div>
      </div>)}
