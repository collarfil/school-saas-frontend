...
export default function Expense() {
  ...
  const [form, setForm] = useState({ amount: "", date: new Date().toISOString().split('T')[0], note: "", school_id: "" });
  ...
  const getSchoolId = () => {
    ...
    } catch (error) { console.error("Error getting school ID:", error); }
    return null;
  };
  ...
  const loadAll = async () => {
    ...
    try {
      const res = await api.get("/expenses", { params: { school_id: schoolId } });
      const expensesData = extractArrayData(res);
      setExpenses(Array.isArray(expensesData) ? expensesData : []);
    } catch (err) {
      console.error("❌ Load expenses error:", err);
      if (err.response?.status === 422) toast.error("School ID is required. Please refresh and try again.");
      else toast.error("Failed to load expense records");
      setExpenses([]);
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this expense record?")) {
      try {
        const schoolId = getSchoolId();
        await api.delete(`/expenses/${id}`, { params: { school_id: schoolId } });
  ...
  const getTableData = () => {
    return expensesArray.map((expense, index) => ({
      id: expense.id,
      amount_formatted: formatAmount(expense.amount),
      date_formatted: expense.date ? new Date(expense.date).toLocaleDateString() : 'N/A',
      note: expense.note || <span className="text-gray-400 italic">No note</span>,
      original: expense,
      index: index
    }));
  };
  ...
  return (
    <div className="text-white p-6">
      <div className="flex justify-between items-center mb-6">
        <div><h2 className="text-2xl font-bold">Expense Records</h2><p className="text-sm text-gray-400">{expensesArray.length} Records</p></div>
        <button onClick={handleOpenModal} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition-colors font-medium flex items-center gap-2" disabled={loading}>
          <Plus className="h-4 w-4" />{loading ? "Loading..." : "Add Expense"}
        </button>
      </div>

      <div className="mb-4 p-3 bg-slate-800 rounded text-sm">
        <div className="text-gray-300"><strong>School ID:</strong> {getSchoolId() || "Not found"}<br /><strong>Expenses Count:</strong> {expensesArray.length}</div>
      </div>

      <DataTable columns={tableColumns} data={getTableData()} loading={loading} title="Expense Records" searchPlaceholder="Search by note..." onSearch={handleTableSearch} actions={renderActions} />
      ...
      {show && (<div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50 p-4">
        <div className="bg-slate-800 p-6 rounded-lg w-full max-w-md border border-slate-700">
          <h3 className="text-xl font-semibold mb-4">{editId ? "Edit Expense Record" : "Add New Expense"}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 bg-slate-700/50 rounded text-sm"><p className="text-gray-300"><strong>Note:</strong> This expense will be recorded for your school (School ID: {getSchoolId()})</p></div>
