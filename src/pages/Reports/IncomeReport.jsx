...
import { Printer, Download, TrendingUp, Calendar, FileText, Search } from 'lucide-react';
...
export default function IncomeReport() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      setReportData({
        income: incomeData,
        total_income: incomeData.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0)
      });
      setShowReport(true);
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('income-report-content');
  ...
  const handleDownloadPDF = async () => {
    const element = document.getElementById('income-report-content');
    if (!element) return;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-gray-900 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
          <div>
            <h2 className="text-2xl font-bold text-white">Income Report</h2>
            <p className="text-gray-400 mt-1">View all income transactions</p>
          </div>
        </div>
        ...
        {showReport && reportData && (
          <div className="bg-white rounded-lg overflow-hidden">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center flex-wrap gap-2 no-print">
              <div>
                <span className="text-sm font-medium text-gray-600">Income Report</span>
            ...
            <div id="income-report-content" ref={reportRef} className="p-6">
              <div className="mb-6">
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl border border-green-200 mb-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-green-700 font-medium">Total Income</p>
                    <p className="text-3xl font-bold text-green-800 mt-1">{formatCurrency(reportData.total_income)}</p>
                  </div>
                  <div className="p-4 bg-green-200 rounded-full">
                    <TrendingUp className="h-8 w-8 text-green-700" />
                  </div>
                </div>
                <p className="text-sm text-green-600 mt-2">{reportData.income.length} income transactions</p>
              </div>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {reportData.income.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-800">{item.note || 'Income'}</td>
                          <td className="px-6 py-4 text-sm text-green-600 font-medium text-right">{formatCurrency(item.amount)}</td>
                        </tr>
                      ))}
                      {reportData.income.length === 0 && (
                        <tr>
                          <td colSpan="4" className="px-6 py-8 text-center text-gray-500">No income records found</td>
                        </tr>
                      )}
                    </tbody>
                    {reportData.income.length > 0 && (
                      <tfoot className="bg-gray-50 font-semibold">
                        <tr>
                          ...
                          <td className="px-6 py-3 text-sm text-green-700 text-right">{formatCurrency(reportData.total_income)}</td>
                        </tr>
                      </tfoot>
                    )}
                  </table>
