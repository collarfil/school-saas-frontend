...
export default function EmployeeGrade() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/employee-grades", { ...form, school_id: schoolId });
    ...
    <div className="p-6">
      ...
      {showModal && (<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div className="bg-slate-800 w-full max-w-md rounded-xl border border-slate-700">
          <div className="p-6 border-b border-slate-700"><h2 className="text-xl font-semibold text-white">Assign Grade to Employee</h2><p className="text-gray-400 text-sm mt-1">Select an employee and grade to assign</p></div>
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-slate-700">
              ...
              <button type="submit" disabled={saveLoading || employees.length === 0 || grades.length === 0} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium disabled:bg-blue-400">{saveLoading ? "Saving..." : "Save Assignment"}</button>
            </div>
          </form>
        </div>
      </div>)}
